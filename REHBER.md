...
#### Mobil Navigasyon
768px altında header'daki nav linkleri gizlenir, yerine ☰ hamburger butonu gelir. Tıklayınca tam ekran karanlık overlay menü açılır — dikey sıralı linkler, ✕ kapatma butonu. Her link tıklandığında menü otomatik kapanır.

#### Aktif Sayfa Göstergesi
Header'daki nav linklerinden hangisinin sayfasındaysan, altında ince bir accent rengi çizgi belirir. `data-path` attribute ile eşleşme yapılır.

#### Defter Çizgileri
Mektup sayfalarında post body'nin arka planında çok soluk yatay çizgiler — `repeating-linear-gradient` ile. Gerçek bir defterin çizgili sayfası gibi.

#### Paragraf Ayırıcı
Mektuplarda her iki paragraf arasında ortada soluk bir `·` noktası. Osmanlı el yazması defterlerdeki paragraf ayırıcıları gibi. `p + p::before` pseudo-element ile.

#### Mektup Sonu İmzası
Her mektubun son satırından sonra sağa yaslı, soluk, italik: *"— münzevî"*. Fiziksel mektuplardaki imza geleneği.

#### Rüzgar Salınımı
Anasayfadaki "rüzgara bırakılan sayfa" alıntısı hafifçe sağa-sola sallanır — `windSway` animasyonu ile 4 saniyelik döngü, ±0.3 derece. Rüzgarda kağıt hissi.

#### Sual Defteri Kartları
Her soru artık dashed kenarlıklı bir kart içinde. Hover'da kenarlık accent rengine dönüşür. Sorular arasında daha fazla boşluk — her biri bağımsız bir kart gibi.

#### Muhatap Monogramları
Her muhatap kartının sağ tarafında dev, çok soluk (opacity: 0.04) bir Arapça harf:
- **Sûveyda**: س (sin)
- **Hira**: ح (ha)
- **Cavidan**: ج (cim)
- **Acize**: ع (ayn)
...
#### Halvethâne Nefes Rehberi
Halvethâne'de kademeli açılımlardan önce, büyüyüp küçülen soluk bir daire — nefes al/ver rehberi. 8 saniyelik döngü. Meditasyon hissi.

#### Dolunay Geri Sayım
Dolunay sayfası kilitli olduğunda, bir sonraki dolunaya yaklaşık kaç gün kaldığını gösterir. Hayal kırıklığını beklentiye çevirir.

#### Takvim Yaprağı
Takvim defterindeki günün satırı, düz metin yerine düzensiz kenarlı bir kağıt parçası içinde gösterilir — `clip-path: polygon()` ile yırtılmış kenar hissi. Gölgeli, kağıt dokusu.
...
#### Ağaç Tooltip
Footer'daki büyüyen ağacın üzerine gelindiğinde: *"X yaprak · Y çiçek"* — okuyucunun ağacının durumu.

#### 404 Sayfası
Var olmayan URL'ler için özel sayfa: *"bu sayfa kaybolmuş. belki hiç var olmadı. belki de sen yanlış yere baktın. ama aramak güzeldir."* + defterin başına dön linki.
